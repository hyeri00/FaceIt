...
class FaceView: UIView {
    @IBInspectable // inspecter 설정창에서도 볼 수 있다.
    var scale: CGFloat = 0.90 { didSet { setNeedsDisplay() } }  // 90%
    @IBInspectable
    var mouthCurvature: Double = 1.0 { didSet { setNeedsDisplay() } }
    // 1 함박웃음, -1 찡그린, mouthCurvature=굽은 비율
    @IBInspectable
    var eyesOpen: Bool = false { didSet { setNeedsDisplay() } }
    @IBInspectable
    var eyeBrowTilt: Double = -0.5 { didSet { setNeedsDisplay() } }   // -1 주름가득, 1 눈썹 풀리게
    @IBInspectable
    var color: UIColor = UIColor.blue { didSet { setNeedsDisplay() } }
    @IBInspectable
    var lineWidth: CGFloat = 5.0 { didSet { setNeedsDisplay() } }    // 모든 선 너비를 변수를 사용해 5.0으로 만듬
    ...
    private func pathForCircleCenteredAtPoint(midPoint: CGPoint, radius: CGFloat) -> UIBezierPath {
        let path =  UIBezierPath(arcCenter: midPoint, radius: radius, startAngle: 0.0,
